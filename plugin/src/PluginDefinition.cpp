...
void cmdSignOut() {
    if (MessageBoxW(g_nppData._nppHandle, L"Sign out of Notepad++ Sync on this device?", kPluginName,
...
void cmdConflicts() {
    Dialogs::showConflicts(g_nppData._nppHandle, *g_engine);
}
void cmdSettings() {
    Dialogs::showSettings(g_nppData._nppHandle, *g_engine);
}
void cmdAbout() {
    Dialogs::showAbout(g_nppData._nppHandle);
}
...
void cmdPauseSync() {
    ...
    ::SendMessageW(g_nppData._nppHandle, NPPM_SETMENUITEMCHECK, (WPARAM)g_funcItems[CmdPauseSync]._cmdID,
                   (LPARAM)paused);
}
...
void pluginBeNotified(SCNotification* notify) {
    ...
    case NPPN_FILESAVED: {
        wchar_t path[MAX_PATH] = {0};
        ::SendMessageW(g_nppData._nppHandle, NPPM_GETFULLPATHFROMBUFFERID, (WPARAM)notify->nmhdr.idFrom,
                       (LPARAM)path);
        if (path[0] && g_engine)
            g_engine->onFileSaved(path);
        break;
    }
...
LRESULT pluginMessageProc(UINT, WPARAM, LPARAM) {
    return TRUE;
}
