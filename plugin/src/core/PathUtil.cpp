...
bool PathUtil::normalizeRelative(const std::string& in, std::string& out) {
    out.clear();
    if (in.empty() || in.size() > 4096) return false;

    std::string s = in;
    std::replace(s.begin(), s.end(), '\\', '/');

    // Reject absolute paths, drive letters, UNC, and NULs up front.
    if (s[0] == '/' || (s.size() > 1 && s[1] == ':')) return false;
    ...
    while (std::getline(ss, seg, '/')) {
        ...
        if (seg == "..") {
            if (segs.empty()) return false; // escapes the root
            segs.pop_back();
            continue;
        }
        segs.push_back(seg);
    ...
    if (hasIllegalChars(out) || hasReservedName(out)) return false;
    return true;
}
...
bool PathUtil::hasIllegalChars(const std::string& relPath) {
    ...
    for (unsigned char c : relPath) {
        ...
        if (illegal.find(static_cast<char>(c)) != std::string::npos) return true;
    }
    ...
    if (!base.empty() && (base.back() == '.' || base.back() == ' ')) return true;
    return false;
}
...
bool PathUtil::joinInsideRoot(const std::wstring& absRoot, const std::string& relNormalized,
    ...
    std::string rel;
    if (!normalizeRelative(relNormalized, rel)) return false;
