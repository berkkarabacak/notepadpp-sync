...
std::vector<Edit> diffLines(const std::vector<std::string>& from, const std::vector<std::string>& to,
                            bool& ok) {
    ok = true;
    ...
    if (n == 0 && m == 0)
        return edits;
    if (n > 0 && m > 0 && n * m > kMaxAutoMergeCells) {
        ok = false;
        return edits;
    }
    ...
    for (size_t i = n; i-- > 0;)
        for (size_t j = m; j-- > 0;)
            at(i, j) = (from[i] == to[j]) ? at(i + 1, j + 1) + 1 : std::max(at(i + 1, j), at(i, j + 1));
...
MergeResult ThreeWayMerge::merge(const std::string& base, const std::string& local,
    ...
    if (!okL || !okR)
        return result; // too large to auto-merge safely
    ...
    auto baseSlice = [&](size_t a, size_t b) {
        ...
        for (size_t k = a; k < b && k < nBase; ++k)
            v.push_back(baseLines[k]);
        return v;
    };
    ...
    while (basePos < nBase || iL < editsL.size() || iR < editsR.size()) {
        ...
        if (iL < editsL.size())
            nextStart = std::min(nextStart, editsL[iL].aStart);
        if (iR < editsR.size())
            nextStart = std::min(nextStart, editsR[iR].aStart);

        if (nextStart == SIZE_MAX) {
            while (basePos < nBase) {
                out += baseLines[basePos++];
                out += eol;
            }
            break;
        }
        while (basePos < nextStart && basePos < nBase) {
            out += baseLines[basePos++];
            out += eol;
        }
        ...
        auto build = [&](const std::vector<Edit>& edits, size_t from, size_t to) {
            ...
            for (size_t k = from; k < to; ++k) {
                while (cursor < edits[k].aStart && cursor < nBase)
                    rep.push_back(baseLines[cursor++]);
                for (const auto& l : edits[k].lines)
                    rep.push_back(l);
                cursor = edits[k].aEnd;
            }
            while (cursor < regionEnd && cursor < nBase)
                rep.push_back(baseLines[cursor++]);
            return rep;
        };
