...
Bytes Crypto::random(size_t len) {
    Bytes out(len);
    if (len == 0) return out;
    if (BCryptGenRandom(nullptr, out.data(), static_cast<ULONG>(len),
                        BCRYPT_USE_SYSTEM_PREFERRED_RNG) != 0)
        throw std::runtime_error("BCryptGenRandom failed");
    return out;
}

Bytes Crypto::generateMasterKey() { return random(kMasterKeyLen); }

Bytes Crypto::aesGcmCrypt(bool encrypting, const Bytes& key, const Bytes& nonce,
    ...
    if (BCryptGenerateSymmetricKey(aesProvider(), &kh, nullptr, 0,
                                   const_cast<PUCHAR>(key.data()),
                                   static_cast<ULONG>(key.size()), 0) != 0)
        throw std::runtime_error("BCryptGenerateSymmetricKey failed");

    Bytes producedTag;
    if (encrypting) producedTag.resize(kGcmTagLen);
    ...
    NTSTATUS st;
    if (encrypting) {
        st = BCryptEncrypt(kh, const_cast<PUCHAR>(input.data()), static_cast<ULONG>(input.size()),
                           &info, nullptr, 0, out.data(), static_cast<ULONG>(out.size()),
                           &produced, 0);
    } else {
        st = BCryptDecrypt(kh, const_cast<PUCHAR>(input.data()), static_cast<ULONG>(input.size()),
                           &info, nullptr, 0, out.data(), static_cast<ULONG>(out.size()),
                           &produced, 0);
    }
    BCryptDestroyKey(kh);
    if (st != 0) {
        if (!encrypting) return {}; // auth failure -> empty
        throw std::runtime_error("BCryptEncrypt failed");
    }
    out.resize(produced);
    if (encrypting) tagOut = std::move(producedTag);
    return out;
}
...
bool Crypto::decrypt(const Bytes& key, const Bytes& envelope, const std::string& aad, Bytes& plaintextOut) {
    ...
    if (envelope.size() < headerLen) return false;
    if (std::memcmp(envelope.data(), kEnvelopeMagic, 4) != 0) return false;
    if (envelope[4] != kAlgAes256Gcm) return false;
    ...
    if (out.empty() && !ct.empty()) return false;
...
std::string Crypto::sha256Hex(const Bytes& data) {
    ...
    if (st == 0) st = BCryptHashData(hh, const_cast<PUCHAR>(data.data()),
                                     static_cast<ULONG>(data.size()), 0);
    if (st == 0) st = BCryptFinishHash(hh, digest, sizeof(digest), 0);
    if (hh) BCryptDestroyHash(hh);
    BCryptCloseAlgorithmProvider(ah, 0);
    if (st != 0) throw std::runtime_error("sha256 failed");
...
Bytes Crypto::deriveKeyFromCode(const std::string& code, const Bytes& salt, uint32_t iterations) {
    BCRYPT_ALG_HANDLE ah = nullptr;
    if (BCryptOpenAlgorithmProvider(&ah, BCRYPT_SHA256_ALGORITHM, nullptr,
                                    BCRYPT_ALG_HANDLE_HMAC_FLAG) != 0)
        throw std::runtime_error("hmac provider failed");
    Bytes key(kMasterKeyLen);
    NTSTATUS st = BCryptDeriveKeyPBKDF2(ah,
        ...
        iterations, key.data(), static_cast<ULONG>(key.size()), 0);
    BCryptCloseAlgorithmProvider(ah, 0);
    if (st != 0) throw std::runtime_error("PBKDF2 failed");
    return key;
}
...
std::string Crypto::base64UrlEncode(const Bytes& data) {
    ...
    if (i < data.size()) {
        ...
        out.push_back(tbl[(v >> 18) & 63]);
        out.push_back(tbl[(v >> 12) & 63]);
        if (two) out.push_back(tbl[(v >> 6) & 63]);
    }
    return out;
}
...
bool Crypto::normalizeRecoveryKey(const std::string& in, std::string& normalizedOut) {
    ...
    for (size_t i = 0; i < body.size(); ++i) {
        if (i > 0 && i % 4 == 0) normalizedOut.push_back('-');
