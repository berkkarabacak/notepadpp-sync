...
struct Stmt
{
    ...
    bool bind(int i, int64_t v) {
        return sqlite3_bind_int64(st, i, v) == SQLITE_OK;
    }
    bool bindW(int i, const std::wstring& v) {
        return bind(i, PathUtil::wideToUtf8(v));
    }
...
bool LocalDb::open(const std::wstring& path) {
    ...
    if (sqlite3_open_v2(u8.c_str(), &db_, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX,
                        nullptr) != SQLITE_OK) {
        close();
        return false;
    }
...
bool LocalDb::addSyncRoot(const std::string& id, const std::wstring& absPath, bool isFolder) {
    Stmt st(db_, "INSERT INTO sync_roots(id,abs_path,is_folder) VALUES(?,?,?) ON CONFLICT(id) DO UPDATE SET "
...
std::vector<std::pair<std::string, std::wstring>> LocalDb::listSyncRoots() {
    ...
    if (!st.st)
        return out;
    while (st.stepRow())
        out.emplace_back(st.colText(0), PathUtil::utf8ToWide(st.colText(1)));
    return out;
}
...
bool LocalDb::upsertFile(const LocalFileState& f) {
    Stmt st(db_,
            "INSERT INTO "
            "files(rel_path,file_id,sync_root,local_hash,remote_hash,remote_version,version_vector,deleted,"
            "modified_local)"
            " VALUES(?,?,?,?,?,?,?,?,?)"
            ...
            " version_vector=excluded.version_vector, deleted=excluded.deleted, "
...
std::optional<LocalFileState> LocalDb::getFile(const std::string& relPath) {
    Stmt st(db_, "SELECT "
                 "file_id,sync_root,local_hash,remote_hash,remote_version,version_vector,deleted,modified_"
                 "local FROM files WHERE rel_path=?");
    if (!st.st || !st.bind(1, relPath))
        return std::nullopt;
    if (!st.stepRow())
        return std::nullopt;
    LocalFileState f;
...
std::vector<LocalFileState> LocalDb::allFiles() {
    Stmt st(db_, "SELECT "
                 "rel_path,file_id,sync_root,local_hash,remote_hash,remote_version,version_vector,deleted,"
                 "modified_local FROM files");
...
bool LocalDb::updateRemoteState(const std::string& relPath, const std::string& remoteHash,
    ...
    return st.st && st.bind(1, remoteHash) && st.bind(2, remoteVersion) && st.bind(3, vv.toJson()) &&
           st.bind(4, relPath) && st.stepDone();
}
...
bool LocalDb::enqueueOp(const PendingOp& op) {
    // Coalesce: an existing queued op for the same path is replaced.
    {
        ...
        if (del.st) {
            del.bind(1, op.relPath);
            del.bind(2, op.kind);
            sqlite3_step(del.st);
        }
    }
    Stmt st(db_, "INSERT INTO pending_ops(kind,rel_path,new_rel_path,queued_at,retry_count,last_error) "
                 "VALUES(?,?,?,?,0,'')");
    return st.st && st.bind(1, op.kind) && st.bind(2, op.relPath) && st.bind(3, op.newRelPath) &&
           st.bind(4, nowUnix()) && st.stepDone();
}

std::vector<PendingOp> LocalDb::pendingOps() {
    Stmt st(
        db_,
        "SELECT id,kind,rel_path,new_rel_path,queued_at,retry_count,last_error FROM pending_ops ORDER BY id");
...
bool LocalDb::addConflict(const ConflictState& c) {
    Stmt st(
        db_,
        "INSERT INTO conflicts(file_id,rel_path,remote_version,local_copy_path,remote_blob_hash,detected_at)"
    ...
    return st.st && st.bind(1, c.fileId) && st.bind(2, c.relPath) && st.bind(3, c.remoteVersion) &&
           st.bind(4, c.localCopyPath) && st.bind(5, c.remoteBlobHash) && st.bind(6, nowUnix()) &&
           st.stepDone();
}

std::vector<ConflictState> LocalDb::conflicts() {
    Stmt st(db_, "SELECT file_id,rel_path,remote_version,local_copy_path,remote_blob_hash,detected_at FROM "
                 "conflicts ORDER BY detected_at DESC");
...
bool LocalDb::setLastChangeSeq(int64_t seq) {
    return setMeta("last_change_seq", std::to_string(seq));
}
