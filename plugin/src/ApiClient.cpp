...
bool parseUrl(const std::string& url, ParsedUrl& out) {
    ...
    if (u.rfind("https://", 0) == 0) { out.https = true; u = u.substr(8); }
    else if (u.rfind("http://", 0) == 0) { out.https = false; u = u.substr(7); }
    else return false;
    size_t slash = u.find('/');
...
std::string headerValue(const std::string& allHeaders, const std::string& name) {
    ...
    if (pos == std::string::npos) {
        if (allHeaders.rfind(name + ":", 0) == 0) pos = 0 - name.size() - 1; // first header
        else return {};
    }
...
ApiResponse ApiClient::request(const std::string& method, const std::string& path,
                               const json* body, bool authed, bool retryOn401) {
    ApiResponse resp;
    ParsedUrl pu;
    ...
    if (!hSession) { resp.transportError = "WinHttpOpen failed"; return resp; }
    ...
    if (!hConnect) { WinHttpCloseHandle(hSession); resp.transportError = "WinHttpConnect failed"; return resp; }

    std::string fullPath = pu.basePath + path;
    std::wstring wPath = widen(fullPath);
    HINTERNET hReq = WinHttpOpenRequest(hConnect, widen(method).c_str(), wPath.c_str(),
                                        nullptr, WINHTTP_NO_REFERER,
                                        WINHTTP_DEFAULT_ACCEPT_TYPES,
                                        pu.https ? WINHTTP_FLAG_SECURE : 0);
    if (!hReq) {
        WinHttpCloseHandle(hConnect); WinHttpCloseHandle(hSession);
        resp.transportError = "WinHttpOpenRequest failed"; return resp;
    }
    ...
    if (!proto.empty()) resp.serverProtocol = std::atoi(proto.c_str());
    ...
    if (!resp.rawBody.empty()) {
        try { resp.body = json::parse(resp.rawBody); } catch (...) { resp.body = json(); }
    }
...
bool ApiClient::refreshAccessToken() {
    ...
    accessToken_ = access;
    refreshToken_ = refresh;
    if (onTokensRotated) onTokensRotated(access, refresh);
    return true;
}
...
ApiResponse ApiClient::registerAccount(const std::string& email, const std::string& password,
    ...
    if (r.status == 200) setTokens(r.body.value("access_token", ""), r.body.value("refresh_token", ""));
    return r;
}
...
ApiResponse ApiClient::listDevices() { return request("GET", "/devices", nullptr, true); }
...
ApiResponse ApiClient::listFiles() { return request("GET", "/sync/files", nullptr, true); }
...
ApiResponse ApiClient::getSession() { return request("GET", "/session", nullptr, true); }
...
void WsClient::run() {
    ...
    while (impl_->running) {
        ...
        if (token.empty()) {
            for (int i = 0; i < 20 && impl_->running; ++i) Sleep(500);
            continue;
        }

        ParsedUrl pu;
        if (!parseUrl(baseUrl_, pu)) return;
        ...
        do {
            ...
            hReq = WinHttpOpenRequest(hConnect, L"GET", widen(pu.basePath + "/ws?token=" + token).c_str(),
                                      ...
                                      (pu.https ? WINHTTP_FLAG_SECURE : 0));
            if (!hReq) break;
            if (!WinHttpSetOption(hReq, WINHTTP_OPTION_UPGRADE_TO_WEB_SOCKET, nullptr, 0)) break;
        ...
        for (int i = 0; i < backoffSec * 2 && impl_->running; ++i) Sleep(500);
